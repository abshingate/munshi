...
Invariants that apply specially here: nothing is written to Tally without an
in-conversation user confirmation (enforced by prompt policy + the summary
workflow); `repair.ps1` restarts the app only when its file hash changes, so
repairs don't drop in-flight chats; the API key lives only in
`C:\TallyAI\data\config.json` on the VM.
