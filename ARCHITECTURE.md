...
`vm/app/` is a Node.js web app deployed to `C:\TallyAI` and run as the
`TallyAIApp` scheduled task (SYSTEM, on boot), HTTPS on 8444 with a
self-signed cert generated by `repair.ps1`. Layers:

- `server.js` — zero-framework HTTP layer: static UI, passcode auth (HMAC
  session cookie), `/api/setup|login|status|chat|history|refresh|reset`.
  Config and chat history persist in `C:\TallyAI\data\` (excluded from the
  code sync, so updates never wipe them).
- `lib/llm.js` + `lib/provider-anthropic.js` — pluggable provider interface;
  the Anthropic implementation uses the official SDK, `claude-opus-5`,
  server-side refusal fallbacks, and non-streaming calls (16K max_tokens).
- `lib/agent.js` — persona ("Munshi", 20-year Indian CA), tool definitions,
  knowledge builder (company + ledger snapshot cached to disk and injected as
  a cache-controlled system block), and the manual agentic loop (max 12
  turns, parallel tool_use handled, all tool_results returned in one user
  message, refusal stop_reason handled).
- `lib/tally.js` — TallyPrime XML gateway client (localhost:9000): collection
  exports (companies, ledgers), Day Book export, and Import Data writes
  (ledger + voucher creation with a debit=credit balance check).

- `lib/drafts.js` — the **only** code path that writes to Tally. Lifecycle:
  `pending → posting → posted|failed` (or `pending → rejected`). The model's
  `propose_entry` tool creates a draft; the UI renders it as a card; posting
  runs only from the user's Confirm tap (`/api/confirm`). Guarantees:
  synchronous status flip before any I/O (double-tap → 409), a pre-post
  day-book marker check (survives a lost response without double-posting),
  ledger existence verified/created first, ≥₹1 lakh requires typed-amount
  confirmation in the UI, verify-after-write via day-book read-back of the
  `[M-<id>]` narration marker, bill images filed to
  `C:\TallyData\Documents\FY<yy>\<MM-Month>\` with the path echoed into the
  narration, an append-only `audit.log`, and a knowledge refresh after every
  post.

Invariants that apply specially here: **the model cannot write to Tally** —
`propose_entry` is its only write-adjacent tool and posting is gated on the
user's tap in `drafts.js`; `repair.ps1` restarts the app only when its file
hash changes, so repairs don't drop in-flight chats; the API key lives only
in `C:\TallyAI\data\config.json` on the VM; `C:\TallyAI\data` and
`C:\TallyData\Documents` are excluded from code syncs and covered by nightly
snapshots.
